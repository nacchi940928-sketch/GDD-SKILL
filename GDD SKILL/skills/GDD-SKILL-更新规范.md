...
| 类型 | 位置 | 状态 |
|------|------|------|
| **阶段 Skill** | `skills/gdd/p0~p6-*/SKILL.md` | 有骨架，随项目打磨 |
| **维度 Skill** | `skills/tech/*`、`skills/ux/*`、`skills/design/*` | 按领域增量建设 |
...
① 真源输入          ② 跑管线 / 程序拆解        ③ 发现缺口或反模式
   参考/docx            产出/ 或 程序交付 GDD          评审会 / diff / QA
        │                        │                        │
        └────────────────────────┴────────────────────────┘
                                 ▼
...
4. **是格式问题还是业务规则？** 业务规则 → `02/规则/`；格式/协作 → Skill
...
| **S7 验证** | 用**新功能**或**回归跑一轮** P2/P3，对照自检清单 | `产出/…/logs/` 记录 |
...
| **真源** | `参考/超级鸡马-嵌入表格/`、`产出/超级鸡马/02-…/AB2_ChickenHorseItem.md` |
