# 提示词总索引

> 路径规范：`{feature_root}` = `产出/{功能名}`。详见 [使用指导.md](../使用指导.md)。  
> **Prompt 清单与门禁** → [规格交付库.md](规格交付库.md)（复制 Prompt 用本文件，不必翻各阶段 README）。

---

## 两部分 Prompt 体系

| 部分 | 目录 | 定位 | 索引 |
|------|------|------|------|
| **Part 1 策划辅助** | [立项探索/](立项探索/) | 帮策划**写**案（可选） | [立项探索/README.md](立项探索/README.md) |
| **Part 2 规格交付** | [原始策划案/](原始策划案/)～[开发管线/](开发管线/) | 将 01 **拆解**为程序/AI 可执行 GDD（**工作重点**） | **[规格交付库.md](规格交付库.md)** |

```text
Part 1 策划辅助（可选）                    Part 2 规格交付（核心）
立项探索 → 01 草稿              →    原始策划案定稿 → 需求拆解 → **工程真源对齐** → 功能点梳理 → 待策划补充 → 编译验收 → 开发管线
                                          ↑
                                策划 docx 也可直接从「原始策划案」进入
```

**Part 2 交付目标**：无论哪位策划的 01，经统一 Prompt 库产出规范一致的 `02 + 03`。

> **边界**：Prompt 只描述**文档规范**（格式、路径、编号）；具体业务在 `案例/`。`skills/` 变更须维护者确认，AI 跑管线时不得修改。

---

## Part 2 快速入口

| 你要做什么 | 打开 |
|------------|------|
| 了解完整规范、编号、门禁 | **[规格交付库.md](规格交付库.md)** |
| 复制公共头块 | [公共/规格交付-公共头.md](公共/规格交付-公共头.md) |
| 原始策划案（01 定稿） | [原始策划案/README.md](原始策划案/README.md) |
| 需求拆解（02 九步） | [需求拆解/README.md](需求拆解/README.md) |
| **工程真源对齐（02.5）** | [工程真源对齐/README.md](工程真源对齐/README.md) |
| 功能点梳理（03） | [功能点梳理/README.md](功能点梳理/README.md) |
| 待策划补充（04） | [待策划补充/README.md](待策划补充/README.md) |
| **决策稿对账（P4.5 可选）** | [决策稿对账/README.md](决策稿对账/README.md) |
| 编译验收 | [编译验收/README.md](编译验收/README.md) |
| 开发管线 | [开发管线/README.md](开发管线/README.md) |

> **原始策划案～开发管线** 共 25 个提示词正文（含 P4.5 可选 1 步），均已统一 Part 2 六段式结构。

---

## Part 2 阶段索引

| 阶段 | 提示词数 | 规范 Skill | 说明 |
|------|----------|------------|------|
| 原始策划案 | 2 | [p1-original-gdd-原始策划案](../pipeline/p1-original-gdd-原始策划案/SKILL.md) | docx → 01 定稿 |
| **需求拆解** | **9** | [p2-decompose-需求拆解](../pipeline/p2-decompose-需求拆解/SKILL.md) | **核心：02 九步编号化** |
| **工程真源对齐** | **1** | [p2b-program-bind-工程真源对齐](../pipeline/p2b-program-bind-工程真源对齐/SKILL.md) | **P2 后：仓库绑定 / Q-P 待补** |
| **功能点梳理** | **6** | [p3-feature-spec-功能点梳理](../pipeline/p3-feature-spec-功能点梳理/SKILL.md) | **核心：程序主文档 03** |
| 待策划补充 | 2 | [p4-planner-fill-待策划补充](../pipeline/p4-planner-fill-待策划补充/SKILL.md) | 策划回填 04 delta |
| **决策稿对账（P4.5 可选）** | **1** | [gdd-decompose](../gdd-decompose/SKILL.md) §P4.5 | 外部决策稿 diff + 回流 |
| 编译验收 | 4 | [p5-compile-verify-编译验收](../pipeline/p5-compile-verify-编译验收/SKILL.md) | compile + 验收 + **执行日志** |
| 开发管线 | 3 | [p6-pipeline-开发管线](../pipeline/p6-pipeline-开发管线/SKILL.md) | server/client 四件套 |

## Part 2 推荐顺序

```text
原始策划案/Docx转原始案 → 完整性检查 → 策划确认 01
  ↓
需求拆解：数据源 → 系统内容分解 → 规则 → 边界条件 → 状态机 → UI交互 → 红点 → 校验规则 → 验收标准
  ↓
工程真源对齐：按需求摘录字段 → 02.5-工程真源摘录.md（program_repo 可选）
  ↓
功能点梳理：阅读说明 → 功能点拆分（可分批）→ 字段映射 → 验收场景 → 待确认事项 → Feature对齐
  ↓
待策划补充/生成待补充清单 → 策划回填 04 → 策划回填验收 ──┐
  ↑                                                    │ 未通过
  └────────────────────────────────────────────────────┘
  ↓
（可选）决策稿对账/外部决策稿对账 → 回流 01~04
  ↓
编译验收：职能分轨检查 → 执行编译验收 → 策划验收清单 → 管线执行日志 → 开发管线
```

## 用法

1. 打开 [规格交付库.md](规格交付库.md) 确认本步门禁与 @ 引用
2. 打开对应阶段目录下的 `.md`，**仅复制**「提示词正文」代码块（不含文末示例）
3. 参照文末 **输入内容示例** 填写占位符（工作包名称等）并 `@` 引用必读文件
4. 检查产出落在 `{feature_root}/` 正确子目录

## 编译

```bash
python skills/tools/compile.py 产出/{功能名} --workflow skills/workflows/{项目}.workflow.json
```

产出 `{feature_root}/开发文档/程序包.md | 测试包.md | 策划包.md`
