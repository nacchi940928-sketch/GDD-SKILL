...
| **L2 真项目（超级鸡马）** | 🟡 **~70%** | 02/03 主文档完成；04 大量待填；开发管线 未启动 |
| **L2 对照样例（竞技场）** | 🟢 **~90%** | 全链路曾跑通；仅保留 02-03 单文档 |

**一句话**：**「怎么写、怎么编译」的规范与工具已基本就绪**；**「策划填完、程序开干」在超级鸡马上仍卡在 待策划补充 待办与 开发管线 未执行**。
...
|------|--------|----------------|----------|----------|------------|------|
| **立项探索** 立项 | 立项探索·项目发现~4 ✅ | p0-discovery-立项探索 ✅ | ✅ | — 跳过 | — | 可选；有 docx 直进 原始策划案 |
| **原始策划案** 01 定稿 | 原始策划案·Docx转原始案~2 ✅ | p1-original-gdd-原始策划案 ✅ | ✅ | ✅ 01 + gap | ✅ | |
| **需求拆解** 02 拆解 | P2×8 ✅ | p2-decompose-需求拆解 ✅ | ✅ | ✅ 单文档 | ✅ 单文档 | 八维度扁平模板已整理 |
| **功能点梳理** 03 功能点 | 功能点梳理·阅读说明~6 ✅ | p3-feature-spec-功能点梳理 ✅ | ✅ | ✅ §0~11 + 附录 A~E | ✅ | 附录 D 专项审计已写 |
| **待策划补充** 04 回填 | 待策划补充·生成待补充清单~2 ✅ | p4-planner-fill-待策划补充 ✅ | ✅ | 🟡 04 多项 ⬜ | 🟡 部分填 | **当前主要阻塞** |
| **编译验收** 编译验收 | 编译验收·职能分轨检查~4 ✅ | p5-compile-verify-编译验收 ✅ | ✅ | 🟡 compile ✅；验收清单 ⬜ | ✅ 三包 + 验收清单 | 超级鸡马缺 编译验收·策划验收清单 产出 |
| **开发管线** 开发管线 | 开发管线·后端管线编排~3 ✅ | p6-pipeline-开发管线 🟡 | 🟡 | ⬜ | ⬜ | 四件套在仓库外，未执行 |
...
1. **完整 Part 2 Prompt 库**（原始策划案～开发管线 + 可选立项探索）与 **pipeline Skill** 七件套  
...
7. **执行日志规范**（编译验收·管线执行日志）+ 看板驱动的 04 回填工作流  
...
| Prompt 瘦身（公共头 + 本步差异） | 低 | 减少 需求拆解 重复粘贴 |
...
| 需求拆解 各维度独立 Skill（规则/状态机/…） | 低~中 | 路线图 ⬜ 项 |
| 功能点梳理 子域 Skill（五块/验收/Feature 映射） | 低 | 现靠 p3-feature-spec-功能点梳理 总规范 |
...
| 项 | 阶段 | 状态 |
|----|------|------|
| 04 配置数值 / 规则边界策划回填 | 待策划补充 | ⬜ 多项 Q-D / Q-R |
| 04-01 字段命名程序确认 + Q-L01 workflow | 待策划补充 | ⬜ |
| 04 回流 → 02/03 更新 | P4→需求拆解/功能点梳理 | 🟡 机制有，未批量执行 |
| 编译验收·策划验收清单 策划验收清单 | 编译验收 | ⬜ |
| 开发管线 后端/前端四件套 | 开发管线 | ⬜ |
| 策划验收「可否进 开发管线 = 是」 | 门禁 | ⬜ |
...
| 项 | 说明 |
|----|------|
| 超级鸡马作为第二份「全链路闭合」样例 | 待 P4+编译验收·策划验收清单 完成后可与竞技场并列 |
...
```text
1. 策划闭合 04（看板 §八 顺序）→ 待策划补充·策划回填验收 验收
2. Agent 回流 02/03 → compile → 新建 logs
3. 补 编译验收·策划验收清单 策划验收清单（超级鸡马/开发文档/）
4. 程序填 workflow skill_configs（Q-L01）
5. 跑 开发管线·后端管线编排 / 开发管线·前端管线编排（仓库外 server/client design）
