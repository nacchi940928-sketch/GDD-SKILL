# GDD 管线步骤表（gdd-decompose · reference）

路径前缀：子项目根（与 `prompts/`、`产出/` 同级）。占位符：`{feature_root}` = `产出/{功能名}`，`{spec_root}` = `{feature_root}/02-03需求拆解与功能点梳理`。

**Skill 中文对照**：`p0-discovery-立项探索` 立项探索 · `p1-original-gdd-原始策划案` 原始策划案 · `p2-decompose-需求拆解` 需求拆解 · `p3-feature-spec-功能点梳理` 功能点梳理 · `p4-planner-fill-待策划补充` 待策划补充 · `p5-compile-verify-编译验收` 编译验收 · `p6-pipeline-开发管线` 开发管线

---

## P0 立项探索（p0-discovery-立项探索 · 可选）

| 步 | Prompt | Skill |
|----|--------|-------|
| 1 | `prompts/立项探索/项目发现.md` | **立项探索** `p0-discovery-立项探索` |
| 2 | `prompts/立项探索/GameDNA定调.md` | 同上 |
| 3 | `prompts/立项探索/功能层级与Skill选型.md` | 同上 |
| 4 | `prompts/立项探索/玩法大纲与01初稿.md` | 同上 |

有 docx 时 **跳过 P0**，直进 P1。

---

## P1 原始策划案（p1-original-gdd-原始策划案）

| 步 | Prompt | pipeline Skill | 产出 |
|----|--------|----------------|------|
| 1 | `prompts/原始策划案/Docx转原始案.md` | **原始策划案** `p1-original-gdd-原始策划案` | `{feature_root}/01-原始策划案/{功能名}/原始策划案.md` |
| 2 | `prompts/原始策划案/完整性检查.md` | 同上 | `01/…/完整性检查-gap清单.md` |

**门禁 → P2**：
- 01 六章齐全
- gap 清单 **P0 决策阻塞 = 0**
- 配置/道具表已全量摘录（01 或 `源表提取/`）
- P1 项已注明登记 04 类别

**禁止**：P0>0 时进入需求拆解；不得用推断默认闭合 P0。

---

## P2 需求拆解（p2-decompose-需求拆解 · 9 步）

> 规范 Skill：**需求拆解** `p2-decompose-需求拆解`

| 步 | 中文 Prompt | 英文目录引用 | 产出章节 | 编号 |
|----|-------------|--------------|----------|------|
| 1 | 数据源 | `prompts/需求拆解/数据源.md` | 02 §6 数据概要 | — |
| 2 | 系统内容分解 | `prompts/需求拆解/系统内容分解.md` | 02 §1.5 | SYS-/CNT- |
| 3 | 规则 | `prompts/需求拆解/规则.md` | 02 §3 | R- |
| 4 | 边界条件 | `prompts/需求拆解/边界条件.md` | 02 §5 | EX/PRE/LIM |
| 5 | 状态机 | `prompts/需求拆解/状态机.md` | 02 §2 | — |
| 6 | UI交互 | `prompts/需求拆解/UI交互.md` | 02 §4 | — |
| 7 | 红点 | `prompts/需求拆解/红点.md` | 02 §6.1 | RD- |
| 8 | 校验规则 | `prompts/需求拆解/校验规则.md` | 03 附录 E.2 | V- |
| 9 | 验收标准 | `prompts/需求拆解/验收标准.md` | 02 §7 | T/B/E |

**规范 Skill**：`pipeline/p2-decompose-需求拆解/SKILL.md`  
**写作规范**：`templates/02-需求拆解-写作规范.md`

**门禁 → P2.5**：
- 02 九步策划可读；§1.5 与 §3 R- 对齐；核心 R- 均有规则说明 + 边界
- **01 覆盖对账**：01 各规则/UI/配置表在 02 有 R-/§4/§6/CNT- 落点（见 p2 SKILL「01 覆盖对账」）
- 竞争/排序类 R- 均有终裁方或【待确认】+ Q-R
- §2 无未标注的静默推断

---

## P2.5 工程真源对齐（p2b-program-bind-工程真源对齐 · 1 步）

> **禁止输入**：`程序反馈/` 目录

| 步 | Prompt | 产出 |
|----|--------|------|
| 1 | `prompts/工程真源对齐/按需求摘录字段.md` | `{spec_root}/02.5-工程真源摘录.md` |

**workflow**：`program_repo`（可选）；无仓库时仍产出 §4 待程序补充清单。

**门禁 → P3**：02.5 存在；未绑定项在 §4；无编造程序字段名；**02 §2 已为 03 章序真源**（含单轮/整局/SD 子阶段）。

---

## P3 功能点梳理（p3-feature-spec-功能点梳理 · 6 步）

> 规范 Skill：**功能点梳理** `p3-feature-spec-功能点梳理`  
> **章序铁律**：`## N.` 顺序 = **02 §2 主流程**；「怎么实现」= **时序表**，禁止伪代码

| 步 | Prompt | 产出 |
|----|--------|------|
| 1 | `prompts/功能点梳理/阅读说明.md` | 03 §0（含可宣讲导航表） |
| 2 | `prompts/功能点梳理/功能点拆分.md` | 03 `## N.` 各章（可分批） |
| 3 | `prompts/功能点梳理/字段映射.md` | 03 附录 A/D/E |
| 4 | `prompts/功能点梳理/验收场景.md` | 03 附录 B |
| 5 | `prompts/功能点梳理/待确认事项.md` | 03 附录 C + 看板 |
| 6 | `prompts/功能点梳理/Feature对齐.md` | Feature 映射（L2） |

**必 @**：`01/…/原始策划案.md`、`02-需求拆解.md`（**§2 章序**）、**`02.5-工程真源摘录.md`**

**门禁 → P4**：03 各章五块 + 流程位置行；§0.2 可宣讲导航；**无时序表外伪代码**；章序与 §2 一致；RoundSettle/MatchSettle/SD 已分列（若适用）；附录 C 已列 Q-P/Q-D/Q-R 与阻塞模块。

---

## P4 待策划补充（p4-planner-fill-待策划补充 · 2 步）

| 步 | Prompt | 产出 |
|----|--------|------|
| 1 | `prompts/待策划补充/生成待补充清单.md` | `{feature_root}/04-待策划补充/{功能名}/` 八文件 + README + 按需 09 |
| 2 | `prompts/待策划补充/策划回填验收.md` | 验收报告 + 回流 02/03 建议 |

**门禁 → P4.5 / P5**：04 骨架齐全；09 默认列不得与 02 已定 R- 相反。

---

## P4.5 外部决策稿对账（可选）

| 步 | 动作 | 产出 |
|----|------|------|
| 1 | 读取用户 @ 的决策闭合表 + 现有 01~04 | diff 分类表 |
| 2 | 回流 02/03/04/01 | 修正 R-/Q-R/Q-D |
| 3 | 写 log + 看板 | `logs/*-决策稿回流.log.md` |

**Prompt**：`prompts/决策稿对账/外部决策稿对账.md`  
**触发词**：对账、程序反馈、业务逻辑表、已经确认、决策稿回流  
**禁止输入**：首次 P1～P4 自动读取  
**门禁 → P5**：决策稿已闭合项无 C 类冲突；03 已同步 02

**P5 可执行率（compile 前汇报）**：统计 R- 总数、已闭合、Q-R/Q-D/Q-P 数；Q-P 须标阻塞模块；**L3 可开发** 要求 Q-P=0 且核心 Q-R 已闭合或 P4.5 回流。

---

## P5～P7

见 `prompts/规格交付库.md` §4～§5。P7 由 `gdd-norm-feedback` 手动触发，不纳入本 Skill 自动顺序。
