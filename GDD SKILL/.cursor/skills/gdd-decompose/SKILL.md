...
{feature_root}/01-原始策划案/{功能名}/源表提取/   ← 大表行级真源（若有）
```

**P1 → P2 硬门禁**：`完整性检查-gap清单.md` 中 **P0 决策阻塞 = 0**；否则停止并列出 P0，不进入需求拆解。
...
| 档位 | 条件 | 适用 |
|------|------|------|
| **L2 · 可评审脚手架** | P2+P3 完成，04 骨架 | 内审、程序预读 |
| **L3 · 可开发包** | L2 + 04 必填闭合 + 02.5 字段绑定 + P4.5（联机/有程序反馈时 **推荐必跑**） | 排期、AI 编码 |
| **L4 · 编译交付** | L3 + compile 三包 + 策划验收 | 正式交付 |

**铁律**：不得将 L2 宣称为「可直接开发」；汇报时须说明阻塞项（P0/Q-P/Q-D/Q-R）数量。
...
12. **03 章序 = 02 §2**：03 `## N.` 顺序与 02 玩家主流程一致；计分按 Phase 拆章（单轮/整局/SD 分列）
13. **03 禁伪代码**：03「怎么实现」用时序表/步骤列表，禁止 FUNCTION/EMIT 块
14. **§1.5 节序 = §2 流程**：SYS 分节按玩家主流程排序，标题含 §2 锚点
15. **02 §2 瘦身**：§2 只写阶段 + 引用 R-；推断标注；参数细节只在 §3
16. **P1 硬门禁**：完整性检查 P0>0 → **禁止进 P2**；不得用 Agent 推断闭合 P0
17. **01 源表全量**：配置/道具/技能行级表须进 01 或 `源表提取/`；P2 每条 R-/CNT- 须能追溯到 01 段落或源表行
18. **静默推断禁止**：01 未写明的流程/规则不得当定论写入 §2/§3；须标【推断·待确认】并登记 Q-R
...
- **阻塞项**：【待确认】、04 未填、完整性检查 **P0**、02.5 Q-P → 列出清单，不编造数值；汇报 **交付档位**（L2/L3/L4）
...
| [references/skill-update-可执行性门禁.md](references/skill-update-可执行性门禁.md) | P1/P2/P5 可执行性门禁 |
