...
> 策划提供原始案 → AI/程序辅助拆解 → **补充清单 ↔ 回填验收** 迭代完善 → 入库交付程序  
> 配套：可复用 Prompt + Skill 知识库、编译器与三层基底架构。
...
| 可复用的 **Prompt + Skill 知识库**，支撑拆解与迭代补全 | 从空白想法一键生成可玩 Demo |
| 对接 design / server / client 仓库的**文档真相来源** | 替代策划撰写原始策划案 |
...
- **交付物**：完整的 `02-需求拆解` + `03-功能点梳理`（及已填完的 `04`），而非半成品或 Demo
